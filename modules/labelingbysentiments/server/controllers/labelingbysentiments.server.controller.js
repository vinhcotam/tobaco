...
var path = require('path'),
  ...
  Labelingbysentiment = mongoose.model('Labelingbysentiment'),
  Newsbysentiment = mongoose.model('Newsbysentiment'),
  errorHandler = require(path.resolve('./modules/core/server/controllers/errors.server.controller')),
  _ = require('lodash');
const { forEach } = require('lodash');
const excel = require('node-excel-export');
/**
 * Create a Labelingbysentiment
 */
exports.create = function(req, res) {
  var labelingbysentiment = new Labelingbysentiment(req.body);
  labelingbysentiment.user = req.user;

  labelingbysentiment.save()
    .then((labelingbysentiment) => {
      res.jsonp(labelingbysentiment);
    })
    .catch((err) => {
      if (err) {
...
/**
 * Show the current Labelingbysentiment
 */
exports.read = function(req, res) {
  // convert mongoose document to JSON
  var labelingbysentiment = req.labelingbysentiment ? req.labelingbysentiment.toJSON() : {};

  // Add a custom field to the Article, for determining if the current User is the "owner".
  // NOTE: This field is NOT persisted to the database, since it doesn't exist in the Article model.
  labelingbysentiment.isCurrentUserOwner = req.user && labelingbysentiment.user && labelingbysentiment.user._id.toString() === req.user._id.toString();

  res.jsonp(labelingbysentiment);
};

/**
 * Update a Labelingbysentiment
 */
exports.update = function(req, res) {
  var labelingbysentiment = req.labelingbysentiment;

  labelingbysentiment = _.extend(labelingbysentiment, req.body);
...
/**
 * Delete an Labelingbysentiment
 */
exports.delete = function(req, res) {
  var labelingbysentiment = req.labelingbysentiment;

  labelingbysentiment.deleteOne()
    .then((labelingbysentiment) => {
      res.jsonp(labelingbysentiment);
    })
    .catch((err) => {
...
exports.insertOrUpdate = async function (req, res) {
  ...
  for (const [key, data] of Object.entries(req.body)) {
    await Labelingbysentiment.updateOne(
      {
...
exports.removeMany = async function (req, res) {
  ...
  for (const [key, value] of Object.entries(data.removeKey)) {
    ...
    await Labelingbysentiment.deleteOne(removeCondition)
      .then((object) => {
        console.log(object);
        ok.push(value);
      })
    ...
    //Labelingbysentiment.remove(removeCondition, function (err) {
    //  if (err != null) {
    //    console.log(err);
    //  } else {
    //  }
...
exports.checkingEditOrRemove = function (req, res) {
  ...
  var listlabel = Labelingbysentiment.aggregate()
    ...
    .lookup({ from: 'newsbysentiments', localField: 'newsdaily._id', foreignField: 'newsdaily', as: 'newsbysentiment' })
    .lookup({ from: 'sentiments', localField: 'newsbysentiment.sentiment', foreignField: '_id', as: 'sentiments' })
  ...
  var countRows = Labelingbysentiment.aggregate()
    .lookup({ from: 'newsdailies', localField: 'newsdaily', foreignField: '_id', as: 'newsdaily' })
    .lookup({ from: 'newsbysentiments', localField: 'newsdaily._id', foreignField: 'newsdaily', as: 'newsbysentiment' })
    .lookup({ from: 'sentiments', localField: 'newsbysentiment.sentiment', foreignField: '_id', as: 'sentiments' })
    .match(condition);
  ...
  if (req.query.typefetch != undefined && req.query.typefetch == 'checking') {
    Labelingbysentiment
      ...
      .then((labelingbysentiments) => {
        //News
        res.jsonp(labelingbysentiments);
      })
      .catch((err) => {
        if (err) {
          return res.status(400).send({
  ...
  if (req.query.typefetch != undefined && req.query.typefetch == 'removedlist') {
    Labelingbysentiment
      ...
      .exec(function (err, labelingbysentiments) {
        if (err) {
          return res.status(400).send({
            message: errorHandler.getErrorMessage(err)
          });
...
exports.list = async function (req, res) {
  ...
  var listlabel = Labelingbysentiment.aggregate()
    ...
    .lookup({ from: 'languagevariables', localField: 'languagevariables', foreignField: '_id', as: 'languagevariables' })
    .unwind('topic')
    .project("_id text created user newsdaily.topic newsdaily.news_url topic.topic_name languagevariables.name sentiments.sentiment_name");//decontruct aray to object
    //.unwind('newsdaily')
...
/**
 * Labelingbysentiment middleware
 */
exports.labelingbysentimentByID = function(req, res, next, id) {

  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(400).send({
      message: 'Labelingbysentiment is invalid'
    });
  }

  Labelingbysentiment.findById(id).populate('user', 'displayName')
    .then((labelingbysentiment) => {
      if (!labelingbysentiment) {
        return res.status(404).send({
          message: 'No Labelingbysentiment with that identifier has been found'
        });
      }
      req.labelingbysentiment = labelingbysentiment;
      next();
    })
    .catch((err) => {
      if (err) {
...
//
exports.labelingbysentimentExport = function (req, res) {
  //
  var condition = {};
  var topics = req.user.topics;
  var topic_id = [];
  ...
  var listlabel = Labelingbysentiment.aggregate()
    ...
    .lookup({ from: 'languagevariables', localField: 'languagevariables', foreignField: '_id', as: 'languagevariables' })
    .unwind('topic')
    .project("_id text newsdaily.npl_date created user newsdaily.topic newsdaily.news_url topic.topic_name languagevariables.name sentiments.sentiment_name crawlerconfigs.config_name news_groups.name websites.website_name")/*;//decontruct aray to object
    //.unwind('newsdaily')
  var listlabel = Labelingbysentiment.aggregate()
    .lookup({ from: 'newsdailies', localField: 'newsdaily', foreignField: '_id', as: 'newsdaily' })
    .lookup({ from: 'topics', localField: 'newsdaily.topic', foreignField: '_id', as: 'topic' })
    ...
    .lookup({ from: 'websites', localField: 'newsdaily.website', foreignField: '_id', as: 'websites' })
    .lookup({ from: 'newsbysentiments', localField: 'newsdaily._id', foreignField: 'newsdaily', as: 'newsbysentiment' })
    .lookup({ from: 'newsdailies', localField: 'newsbysentiments.newsdaily', foreignField: '_id', as: 'group_news' })
    .lookup({ from: 'sentiments', localField: 'newsbysentiment.sentiment', foreignField: '_id', as: 'sentiments' })
    .lookup({ from: 'languagevariables', localField: 'languagevariables', foreignField: '_id', as: 'languagevariables' })
    .unwind('topic')
    .unwind('newsgroup')*/
          ...
          sentiments: {
            ...
            cellFormat: function (value, row) { // <- Renderer function, you can access also any row.property
              if (value.length > 0) {
                var str = "";
                if (value.length == 1) {
                  str = value[0].sentiment_name;
                } else {
                  for (var i = 0; i < value.length; i++) {
                    if (i == value.length - 1) {
                      str += value[i].sentiment_name;
                    } else {
                      str += value[i].sentiment_name + "->";
                    }
                  }
